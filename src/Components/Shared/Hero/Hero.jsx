import { Button } from "flowbite-react";
import { Link } from "react-router-dom";

const Hero = () => {
  return (
    <>
      <div className="">
        <div className="mx-auto max-w-screen-2xl py-12">
          <div className="relative isolate overflow-hidden dark:bg-gray-800  px-6 pt-16 shadow-2xl sm:rounded-3xl sm:px-16 md:pt-24 lg:flex lg:gap-x-20 lg:px-24 lg:pt-0">
            <svg
              viewBox="0 0 1024 1024"
              className="absolute left-1/2 top-1/2 -z-10 h-[64rem] w-[64rem] -translate-y-1/2 [mask-image:radial-gradient(closest-side,white,transparent)] sm:left-full sm:-ml-80 lg:left-1/2 lg:ml-0 lg:-translate-x-1/2 lg:translate-y-0"
              aria-hidden="true"
            >
              <circle
                cx={512}
                cy={512}
                r={512}
                fill="url(#759c1415-0410-454c-8f7c-9a820de03641)"
                fillOpacity="0.7"
              />
              <defs>
                <radialGradient id="759c1415-0410-454c-8f7c-9a820de03641">
                  <stop stopColor="#7775D6" />
                  <stop offset={1} stopColor="#E935C1" />
                </radialGradient>
              </defs>
            </svg>
            <div className="mx-auto max-w-md text-center lg:mx-0 lg:flex-auto lg:py-32 lg:text-left">
              <h2 className="text-3xl font-bold tracking-tight dark:text-white sm:text-4xl">
                Welcome to <span className="font-thin">Khan Blog</span> <br />{" "}
                <span className="text-xl font-thin font-roboto">
                  Your Gateway to Inspiration and Knowledge!
                </span>
              </h2>
              <p className="mt-6 text-lg leading-8 dark:text-gray-300">
                Embark on a journey of discovery and enlightenment today by
                diving into Khan Blog! Explore a wealth of knowledge, insightful
                articles, and captivating stories spanning various topics.
              </p>
              <div className="mt-10 flex items-center justify-center gap-x-6 lg:justify-start">
                <Link to={'/add-blog'}>
                  <Button outline gradientDuoTone="greenToBlue">
                    Get started
                  </Button>
                </Link>
                <a
                  href="/login"
                  className="text-sm font-semibold leading-6 dark:text-white"
                >
                  Join us <span aria-hidden="true">→</span>
                </a>
              </div>
            </div>
            <div className="relative mt-16 h-80 lg:mt-8">
              <img
                className="absolute left-0 top-0 w-[57rem] max-w-none dark:invert rounded-md bg-white/5 ring-1 ring-white/10"
                src="/Banner2.png"
                alt="App screenshot"
                width={1824}
                height={1080}
              />
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Hero;
